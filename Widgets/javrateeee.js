...
function parseDetailPage(detailPageHtml, detailPageUrl) {
  ...
  // 2. 尝试从 iframe 的 src 获取
  if (!videoUrl) {
    videoUrl = $(".player-box iframe").attr("src");
    // 如果是相对路径，尝试补全
    if (videoUrl && !videoUrl.startsWith("http")) {
      try {
        const baseUrl = new URL(detailPageUrl).origin;
        videoUrl = new URL(videoUrl, baseUrl).href;
      } catch (e) {
        console.warn("补全视频地址失败:", e.message);
      }
    }
  }

  // 3. 尝试从 iframe 的 data-src 获取（某些懒加载实现）
  if (!videoUrl) {
    videoUrl = $(".player-box iframe").attr("data-src");
    if (videoUrl && !videoUrl.startsWith("http")) {
      try {
        const baseUrl = new URL(detailPageUrl).origin;
        videoUrl = new URL(videoUrl, baseUrl).href;
  ...
  // 5. 尝试从 video 的 data-src 获取
  if (!videoUrl) {
    videoUrl = $("video").attr("data-src");
  }

  // 6. 如果上述都失败，可以考虑从页面脚本中提取（这里仅作为示例，实际需要根据网站结构调整）
  // 例如，查找包含视频地址的变量： window.playlist = ...
  ...
  // 根据视频地址的域名动态设置 Referer
  let customHeaders = undefined;
  if (videoUrl) {
    try {
      const videoDomain = new URL(videoUrl).hostname;
      customHeaders = { Referer: `https://${videoDomain}/` };
    } catch (e) {
      customHeaders = { Referer: "https://iframe.mediadelivery.net/" };
    }
  }
