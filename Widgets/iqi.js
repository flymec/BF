  ...
  modules: [
    {
      ...
      functionName: "search",
      cacheDuration: 3600,
      params: [{ name: "keyword", title: "关键词", type: "input", description: "关键词" }]
    },
    { title: "电视剧热播", requiresWebView: false, functionName: "getTvHot",      cacheDuration: 1800, params: [] },
    { title: "电影热播",   requiresWebView: false, functionName: "getMovieHot",   cacheDuration: 1800, params: [] },
    { title: "综艺热播",   requiresWebView: false, functionName: "getVarietyHot", cacheDuration: 1800, params: [] }
  ]
};
...
function buildItem(item) {
  ...
  return {
    ...
    type: "url",                          // Forward 标准类型
    title: item.name || item.title || "",
    description: item.shortDesc || item.description || "",
    posterPath: item.imageUrl || item.pic || item.picUrl || "",
    link: link,                           // 详情/播放页地址
    videoUrl: link,                       // 指向同一地址，Forward 会交给 playerType 处理
    playerType: "app"                     // 用系统浏览器/爱奇艺 App 打开
  };
}
...
async function fetchHotByChannel(channelId) {
  ...
  try {
    ...
    var body = res.data;
    var list = (body.data && (body.data.list || body.data.albumList)) || body.list || [];
    if (list.length) {
...
async function search(params) {
  ...
  var url = "https://pcw-api.iqiyi.com/search/recommend/list"
    + "?page_id=1&ret_num=20&channel_id=0&mode=24&data_type=1"
...
async function loadDetail(link) {
  return {
    ...
    link: link,
    videoUrl: link,
    playerType: "app"   // 交给爱奇艺 App 或系统浏览器处理
  };
}
