...
async function parseDetailPage(detailPageHtml, detailPageUrl) {
  ...
  if (videoUrl && !videoUrl.startsWith('http')) {
    videoUrl = new URL(videoUrl, detailPageUrl).href;
  }

  // 如果 videoUrl 指向 iframe 页面，尝试解析真实视频地址
  if (videoUrl && videoUrl.includes('iframe.mediadelivery.net')) {
    try {
      const iframeResponse = await Widget.http.get(videoUrl, {
        headers: {
          Referer: detailPageUrl,
          'User-Agent': getCommonHeaders()['User-Agent']
        }
      });
      if (iframeResponse && iframeResponse.data) {
        const $iframe = Widget.html.load(iframeResponse.data);
        // 查找 video 标签的 src 或 source 标签的 src
        let realVideoSrc = $iframe('video').attr('src') || $iframe('source').attr('src');
        if (realVideoSrc) {
          if (!realVideoSrc.startsWith('http')) {
            realVideoSrc = new URL(realVideoSrc, videoUrl).href;
          }
          videoUrl = realVideoSrc;
        }
      }
    } catch (e) {
      console.error(`解析 iframe 视频源失败:`, e.message);
    }
  }
