...
// 带退避重试的 GET：针对 429（请求过多）和 403（反爬）做指数退避重试
async function httpGetWithRetry(url, options = {}, maxRetries = 4) {
  const baseDelay = 2000; // 2s 起步
  let lastError;

  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      const response = await Widget.http.get(url, options);
      // 部分后端把 429/403 当成功返回，这里再判一次状态码
      const code = response && (response.statusCode || response.status);
      if (code === 429 || code === 403) {
        throw new Error(`Response status code was unacceptable: ${code}.`);
      }
      return response;
    } catch (error) {
      lastError = error;
      ...
      // 429 用更激进的退避；指数退避 + 随机抖动
      const factor = isRateLimited ? 2 : 1;
      const delay =
        baseDelay * factor * Math.pow(2, attempt) +
        Math.floor(Math.random() * 1000);
      console.warn(
        `请求被限流(${isRateLimited ? "429" : "403"})，第 ${attempt + 1} 次重试，等待 ${delay}ms`
      );
      await sleep(delay);
    }
  }
...
async function loadDetail(link) {
  const response = await httpGetWithRetry(link, {
    headers: COMMON_HEADERS,
  });
