...
async function search(params = {}) {
  ...
  try {
    ...
    const $elements = $(".video-wrapper .videoBox").length > 0 
    ...
    $elements.each((index, element) => {
      ...
      const title = $item.find(".videoBox-info .title").text().trim() || $item.attr("title");
      const imgSrc = getCoverImgSrc($item);
    ...
    if (videoItems.length === 0) {
       console.log(`${JAVDAY_LOG_PREFIX} 搜索解析为空，请检查目标网站搜索页的 DOM 结构是否变更。`);
    }
