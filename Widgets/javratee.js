...
/**
 * 将 "2024年3月5日" 格式的日期转换为 "2024-03-05"
 * 若匹配失败返回原字符串
 */
...
let _artistMapFetchPromise = null; // 竞态防护：同一时刻只发一次请求
...
function parseDetailPage(html) {
  ...
  // 封面图（优先从 LD+JSON schema 取，防止 JSON 解析出错）
  let imgSrc = null;
...
/**
 * 根据 path 和 params 计算最终请求 URL
 * 将原来散落的 if/else 链集中到此处维护
 */
function buildRequestUrl(path, params) {
  ...
  // 热门排行：sort_by 优先于 path（fix 原有歧义）
...
async function loadDetail(linkValue) {
  ...
  try {
    ...
    // 步骤 2：从 /Player/V2 取 initialSignedUrl
    let videoUrl = null;
    ...
    if (detailData.iframeSrc) {
      ...
      try {
        ...
        if (playerResponse?.data) {
          ...
          // 优先：initialSignedUrl（完整 token 的 playlist.m3u8）
...
const PAGE_STRATEGIES = [
  // 艺人搜索
  {
    ...
    resolve: async (p) => {
      ...
      if (!found) return null; // 触发"未找到"提示
