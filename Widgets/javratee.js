...
if (videoUrl && videoUrl.startsWith("//")) {
  ...
  relatedItems: relatedItems,
    ...
    playerOptions: {
      ...
      hardwareDecode: true,       // 开启硬解
      ijkOptions: {
        "mediacodec": 1,          // 启用 MediaCodec 硬解
        "opensles": 1,            // 启用 OpenSL ES 音频输出
        "framedrop": 50,          // 帧丢弃阈值
        "max-buffer-size": 1536,  // 最大缓冲区大小
        "packet-buffering": 1,    // 包缓冲
        "start-on-prepared": 1    // 准备完成后自动开始播放
      }
    }
  };
}
...
async function loadDetail(linkValue) {
  ...
  try {
    ...
    return {
      ...
      // 确保 playerOptions 也传递到最终返回
      playerOptions: detailData.playerOptions
    };
  } catch (error) {
