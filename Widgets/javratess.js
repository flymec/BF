...
// [fix #1] 艺人表请求失败后的冷却时间，避免网络故障时反复重试
...
async function fetchArtistMap() {
  ...
  // [fix #1] 失败冷却：上次报错未超过冷却期，直接抛出，不再发起新请求
...
function parseDetailPage(html) {
  ...
  // [fix #6] 用 filter(Boolean).join 避免多余空格
...
// ─── 列表页日期提取 ───────────────────────────────────────────────────────────
// [fix #3] 从列表条目中提取日期文本，独立函数便于维护
// 逻辑：克隆 .mgn-date 元素，移除其中的 SVG 图标，取剩余文本
// 注意：.remove() 返回被移除元素，.end() 回到移除前的集合（即克隆的 .mgn-date）
...
function buildRequestUrl(path, params) {
  ...
  // 艺人页
  // [fix #2] 统一用带分页格式的 URL，第一页 page=1 时同样使用该格式保持一致
    ...
    if (detailData.iframeSrc) {
      ...
      try {
        ...
        if (playerResponse?.data) {
          ...
          if (!videoUrl) {
            // [fix #4] signedUrl 字段本身只是 CDN 基础路径，需要拼接 /playlist.m3u8
            // 与 initialSignedUrl 不同（后者已是完整播放地址）
...
const PAGE_STRATEGIES = [
  ...
  // 标签分类
  // [fix #5] 与厂商策略保持一致，先 decode 再 encode，防止外部传入已编码值时双重编码
  {
